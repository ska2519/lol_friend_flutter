...
class SearchRepository {
  ...
  Future getUserInterests(uid) async {
    ...
    await _firestore.collection('users').doc(uid).get().then((user) {
      ...
      _currentuserProfile.photoUrl = user.get('photoUrl');
      ...
      await _firestore.collection('users').get().then((users) {
        for (var user in users.docs) {
          if ((user.id != uid) &&
            ...
            userProfile.photoUrl = user.get('photoUrl');
  ...
  Future<UserProfile> chooseUser(currentUserId, currentUserName,
      currentUserPhotoUrl, selectedUserId, name, photoUrl) async {
    await _firestore
        .collection('users')
        .doc(selectedUserId)
        .collection('chosenList')
